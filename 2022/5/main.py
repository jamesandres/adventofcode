...
def main(stacks, moves):
    ...
    """
    for move in moves:
        num, from_stack_i, dest_stack_i = move
        for _ in range(num):
            crate = stacks[from_stack_i - 1].pop()
            stacks[dest_stack_i - 1].append(crate)
...
def main2(stacks, moves):
    ...
    for move in moves:
        ...
        for _ in range(num):
            crate = stacks[from_stack_i - 1].pop()
            crane_load.insert(0, crate)
        stacks[dest_stack_i - 1] = stacks[dest_stack_i - 1] + crane_load
    print("".join([stack[-1] for stack in stacks]))
...
def parse_stacks(raw_stacks):
    col_width = 4
    rows = raw_stacks.split("\n")[0:-1]  # Drop the " 1   2   3  " graph labels

    stacks = [[] for _ in range(round(len(rows[0]) / col_width))]

    for row in reversed(rows):
        for i, col in enumerate(chunks(row, col_width)):
            match = re.findall(r"[A-Z]+", col)
            if match:
                stacks[i].append(match[0])

    return stacks


def parse_moves(raw_moves):
    return [tuple(map(int, re.findall(r"\d+", move))) for move in raw_moves.split("\n")]
