...
def main(rucksacks):
    ...
    for rucksack in rucksacks:
        ...
        common_num = [parse_rucksack_item(char) for char in common]
        result.append(sum(common_num))
    print(sum(result))
...
def parse(input):
    ...
    return [parse_rucksack(rucksack) for rucksack in rucksacks]


def parse_rucksack(rucksack):
    length = len(rucksack)
    if length % 2 != 0:
        raise TypeError('Rucksack is not evenly divisible!')
    return (set(rucksack[0:int(length/2)]),
            set(rucksack[int(length/2):length]),)


def parse_rucksack_item(char):
    num = ord(char)
    # Annoyingly lowercase inhabit a higher ASCII range than uppercase, need to swap
    if num >= 97:
        return num - 97 + 1  # 97 is lowercase 'a'
    else:
        return num - 65 + 1 + 26  # 65 is uppercase 'A'
...
def parse2(input):
    ...
    for group in chunks(rucksacks, 3):
        groups.append(tuple(set(rucksack) for rucksack in  group))
    return groups
