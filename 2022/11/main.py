...
def main(monkeys):
    ...
    for r in range(20):
        for i in range(num_monkeys):
            for worry_level in pop_until_empty(monkeys[i].items):
                count += 1
                worry_level = monkeys[i].operation(worry_level)
                worry_level = worry_level // 3
                throw_to_i = monkeys[i].who_to_throw_to(worry_level)
                monkeys[throw_to_i].items.append(worry_level)
                monkeys[i].num_handled += 1

    penultimate, top = sorted([monkey.num_handled for monkey in monkeys])[-2:]
    print(penultimate * top)
...
def main2(monkeys):
    ...
    for r in range(10000):
        for i in range(num_monkeys):
            for worry_level in pop_until_empty(monkeys[i].items):
                # !!! Left off here
                #     Hmmm, the worry magnitudes are growing too large. Need a trick to avoid
                #     doing the entire calculation each time. Somehow keep the magnitudes in
                #     check or only deal with an aspect of it that is needed to compute the
                #     divisibility. Smells like some sort of math magic trick is required..
                if count % 1000 == 0:
                    list_lengths = [len(monkey.items) for monkey in monkeys]
                    print(f'Processing {r=}, {i=}, {count=}, {list_lengths=}..')
                count += 1
                worry_level = monkeys[i].operation(worry_level)
                # worry_level = worry_level // 3
                throw_to_i = monkeys[i].who_to_throw_to(worry_level)
...
def pop_until_empty(items: list):
    try:
        while True:
            yield items.pop(0)
    except IndexError:
...
def parse(raw_input):
    def parse_monkey(raw_monkey):
        _, raw_items, raw_operation, *raw_test = raw_monkey.split('\n')
        # '  Starting items: 79, 98' => [79, 98]
        items = list(map(int, raw_items.split(': ')[1].split(', ')))
        # '  Operation: new = old * 19' => 'old * 19'
        code = raw_operation.split(' = ')[1]
        # A less eval-y approach
        """
        op, operand = raw_operation.split(' = old ')[1].split(' ')
        if op == '*':
            if operand == 'old':
                operation = lambda worry_level: worry_level * worry_level
            else:
                operand = int(operand)
                operation = lambda worry_level: worry_level * operand
        elif op == '+':
            operand = int(operand)
            operation = lambda worry_level: worry_level + operand
        else:
            raise Exception(f'Parse error, unknown operator: {raw_operation}')
        """
        # OMG eval!
        operation = lambda worry_level: eval(code, None, {'old': worry_level})
        # '  Test: divisible by 23' => 23
        divisible_by = int(raw_test[0].split('divisible by ')[1])
        # '    If true: throw to monkey 2' => 2
        if_true_throw_to = int(raw_test[1].split('throw to monkey ')[1])
        # '    If false: throw to monkey 3' => 3
        if_false_throw_to = int(raw_test[2].split('throw to monkey ')[1])
        who_to_throw_to = lambda worry_level: if_true_throw_to if worry_level % divisible_by == 0 else if_false_throw_to
        return Monkey(items, operation, who_to_throw_to)
    return [parse_monkey(raw_monkey) for raw_monkey in raw_input.split('\n\n')]
...
class Monkey:
    items: list[int]
    operation: Callable
    who_to_throw_to: Callable
    num_handled: int = 0

    def __str__(self):
        return f'Monkey({self.num_handled=}, {self.items=}, {self.operation=}, {self.throw_to=})'
