...
def main(buffer):
    ...
    """
    print(find_marker_index(buffer, 4))
...
def main2(buffer):
    ...
    """
    print(find_marker_index(buffer, 14))


def find_marker_index(buffer, window_size):
    for i, window in sliding_window(buffer, window_size):
...
def sliding_window(buffer, window_size):
    for i in range(len(buffer) - window_size + 1):
        yield (i, buffer[i: i + window_size])
