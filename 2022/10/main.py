...
def main(commands):
    ...
    The interesting signal strengths can be determined as follows:
      ...
      - During the 20th cycle, register X has the value 21, so the signal strength is 20 * 21 =
        ...
        6 - 3 + 5 - 1 - 8 + 13 + 4 = 21.)
      - During the 60th cycle, register X has the value 19, so the signal strength is 60 * 19 =
        1140.
      - During the 100th cycle, register X has the value 18, so the signal strength is 100 * 18 =
        1800.
      - During the 140th cycle, register X has the value 21, so the signal strength is 140 * 21 =
        2940.
      - During the 180th cycle, register X has the value 16, so the signal strength is 180 * 16 =
        2880.
      - During the 220th cycle, register X has the value 18, so the signal strength is 220 * 18 =
        3960.
    ...
    for cycle, x in crt_simulator(commands):
        if (cycle - 20) % 40 == 0:
...
def main2(commands):
    ...
    """
    for cycle, sprite_x in crt_simulator(commands):
        crt_x = (cycle - 1) % 40
        if abs(sprite_x - crt_x) <= 1:
            print('#', end='')
        else:
            print(' ', end='')
        if crt_x == 39:
...
def parse(raw_input):
    def parse_command(line):
        ...
        return (command, int(value[0]) if len(value) > 0 else None)
