...
def main2(rounds):
    ...
    def find_score(round):
        if round[1] == 1:
            expected_win_score = 0
        elif round[1] == 2:
            expected_win_score = 3
        else:
            expected_win_score = 6

        # This would be better with a tree, but meh
        for the_round, win_score in WINNER_TABLE.items():
            if win_score == expected_win_score and the_round[0] == round[0]:
                return score(the_round)

    print(sum(find_score(round) for round in rounds))


def score(round):
    return WINNER_TABLE[round] + round[1]
...
def parse_round(round):
    ...
    return tuple([ord(opponent) - 65 + 1, ord(me) - 88 + 1])
