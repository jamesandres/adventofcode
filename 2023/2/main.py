...
def main(lines):
    ...
    for game in games(lines):
        ...
        for set_index, set in enumerate(sets):
            red_ok = set['red'] <= 12
            green_ok = set['green'] <= 13
            blue_ok = set['blue'] <= 14

            sets_possible.append(red_ok and green_ok and blue_ok)
            print(f'  {game_id}:{set_index} possible? {red_ok and green_ok and blue_ok}')

        if all(sets_possible):
...
def main2(lines):
    ...
    for game in games(lines):
        ...
        for set_index, set in enumerate(sets):
            maxes['red'] = max(maxes['red'], set['red'])
            maxes['green'] = max(maxes['green'], set['green'])
            maxes['blue'] = max(maxes['blue'], set['blue'])
...
def games(lines):
    ...
    for line in lines:
        ...
        game_id = int(game_raw.split(' ')[1])
        sets = [dict(defaults + [(cubes_raw.split(' ')[1], int(cubes_raw.split(' ')[0]),) for cubes_raw in set_raw.split(', ')]) for set_raw in sets_raw.split('; ')]
        yield [game_id, sets]
