...
def main(lines):
    ...
    show off her art skills. Consequently, the Elves are having trouble reading the values on the
    document.
...
def main2(lines):
    ...
    for line in lines:
        ...
        first, last = digits[0], digits[-1]
        running_sum += int(ensure_isdigit(first) + ensure_isdigit(last))
    print(running_sum)


def ensure_isdigit(string):
    if string.isdigit():
        return string
    else:
        return {
