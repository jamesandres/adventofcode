"""
Usage: python3 main.py input.txt [2]
"""
import sys
import re
...
def main(schematic):
    ...
    """
    numbers = []
    for search_result, number in part_numbers(schematic):
    ...
    for y in range(len(schematic)):
        ...
        for x in range(len(schematic[0])):
            ...
            if char.isdigit():
                ...
                # Search around x, y to see if there is a symbol. But only bother if we haven't found
                # a symbol yet. No need to duplicate efforts
                if not search_result:
                    search_result = search_for_symbol(schematic, x, y)
            else:
                if len(digits) > 0:
                    number = int(''.join(digits))
                    print(f'{search_result=}, {number=}')
                    if search_result:
                        yield search_result, number
                digits = []
                search_result = False
        if len(digits) > 0:
            number = int(''.join(digits))
...
def search_for_symbol(schematic, x, y):
    for dy in [-1, 0, 1]:
        for dx in [-1, 0, 1]:
            if dx == 0 and dy == 0:
                continue
            ...
            if x2 < 0 or x2 >= len(schematic[0]):
                continue
            if y2 < 0 or y2 >= len(schematic):
                continue

            char = schematic[y + dy][x + dx]

            if not char.isdigit() and char != '.':
                print(f'  {char} at ({x2}, {y2})')
                return (x2, y2,), char
    return False
...
def main2(schematic):
    ...
    for (position, char,), number in part_numbers(schematic):
        if char == '*':
            possible_gears[position].append(number)

    gear_ratios = []
    for position, numbers in possible_gears.items():
        if len(numbers) == 2:
...
if __name__ == "__main__":
    input = open(sys.argv[1], "r").read().rstrip("\n")
